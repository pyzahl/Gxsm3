			...
			if (gtk_dialog_run (GTK_DIALOG (chooser)) == GTK_RESPONSE_ACCEPT){
				...
				if (newpath){
					...
					path = newpath;
                                        ...
                                        g_settings_set_string (gapp->get_as_settings (), "auto-save-folder-netcdf", path);
                                        g_settings_set_string (gapp->get_as_settings (), "auto-save-folder-probe", path);
                                        // xrm.Put ("NC_DataSavePath", path);
					// xrm.Put ("Probe_DataSavePath", path);
				}
			}
			gtk_widget_destroy (chooser);
			return 0;
