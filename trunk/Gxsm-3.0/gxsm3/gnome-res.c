...
void gnome_res_write_user_config (GnomeResPreferences *self){
        ...
        for (res = self->res_def; res->type != GNOME_RES_LAST; ++res){
                ...
                case GNOME_RES_STRING: 
                        if (res->entry)
                                strcpy ((gchar*)res->var, gtk_entry_get_text (GTK_ENTRY (res->entry)));
                        if (res->tmp)
                ...
                case GNOME_RES_INT: 
                        if (res->entry)
                                * ((int*) (res->var)) =  atoi (gtk_entry_get_text (GTK_ENTRY (res->entry)));
                        if (res->tmp)
                ...
                case GNOME_RES_FLOAT_W_UNIT: 
                        if (res->entry)
                                * ((float*) (res->var)) = atof (gtk_entry_get_text (GTK_ENTRY (res->entry)));
                        if (res->tmp)
                ...
                case GNOME_RES_FLOAT_VEC3:
                        vecstr=NULL;
                        if (res->entry)
                                vecstr = g_strdup (gtk_entry_get_text (GTK_ENTRY (res->entry)));
                        if (res->tmp){
        ...
        case GNOME_RES_EDIT_ENTRY:
                {
                        ...
                        gtk_entry_set_text (GTK_ENTRY (variable), resdata);
                        g_free (resdata);
                }
                break;
...
void gnome_res_run_change_user_config (GnomeResPreferences *self, const gchar *dialog_title){
        ...
        gtk_box_pack_start (GTK_BOX ( gtk_dialog_get_content_area (GTK_DIALOG (self->dialog))),
                            notebook, TRUE, TRUE, GXSM_WIDGET_PAD);
...
GnomeResNewUserDruid *gnome_res_new_user_druid_new_with_vals (GnomeResEntryInfoType *res_def,
        ...
        gtk_container_add (GTK_CONTAINER (self->window), self->druid);
