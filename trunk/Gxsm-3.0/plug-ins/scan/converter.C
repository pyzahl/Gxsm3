...
GxsmPlugin *get_gxsm_plugin_info ( void ){ 
  converter_pi.description = g_strdup_printf(N_("Gxsm converter file converter %s"), VERSION);
  return &converter_pi; 
}
...
static void converter_query(void)
{
  if(converter_pi.status) g_free(converter_pi.status); 
  converter_pi.status = g_strconcat (
				     N_("Plugin query has attached "),
				     converter_pi.name, 
				     N_(": File converter is ready to use"),
				     NULL);
}
...
void converter::create_full_path(gchar* target, const gchar* source_directory,
...
void converter::ConvertDir(converterData* work_it, const gchar* current_dir)
{
...
void converter::DoConvert(gchar *pathname,gchar *outputname)
{
  ...
  if ((Ch = readToAct(pathname)) < 0)
    PI_DEBUG(DBG_L2,"readToAct: Conversion of " << pathname << "failed");
                  
  else {
    if (!writeFromCh(Ch, outputname))
      PI_DEBUG(DBG_L2,"Converted: " << pathname << " to " << outputname);
    else 
      PI_DEBUG(DBG_L2,"writeFromCh: Conversion of " << pathname << "failed"); 		       
  }
...
gint  converter::readToAct( gchar *fname){
  ...
  if (!(strstr(fname, ".nc") || strstr(fname, ".NC"))){
    ...
    else ret =-1; //Error: no import filter found!
...
{ 
  topdata = new converterData(getenv("PWD"),getenv("PWD"),"*.nc", "top");
...
void converterControl::run()
{
  ...
  gtk_box_pack_start (GTK_BOX (GTK_WIDGET (DestDir_button)), variable, TRUE, TRUE,0 );
  ...
  g_signal_connect (G_OBJECT (help), "clicked",
		    G_CALLBACK (show_info_callback),
		    (void*)(N_("Enter the file format suffix without any special characters")));
  ...
  g_signal_connect(G_OBJECT(toggle_recursive), "clicked",
		   G_CALLBACK(converterControl::recursive_click),
		   this);
  ...
  topdata->sourceDir = gtk_file_chooser_get_current_folder (GTK_FILE_CHOOSER (SrcDir_button));
  ...
  topdata->destDir = gtk_file_chooser_get_current_folder (GTK_FILE_CHOOSER (DestDir_button));
  ...
  topdata->writeFormat = g_strdup(gtk_entry_get_text (GTK_ENTRY (DestMask)));   
...
void converterControl::recursive_click(GtkWidget* widget, gpointer userdata)
{
  ((converterControl*) userdata)->topdata->m_recursive = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
}


void converterControl::overwrite_target_click(GtkWidget* widget, gpointer userdata)
{
  ((converterControl*) userdata)->topdata->m_overwrite_target = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
}


void converterControl::create_subdirs_click(GtkWidget* widget, gpointer userdata)
{
  ((converterControl*) userdata)->topdata->m_create_subdirs = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
}
...
converterData :: converterData(const gchar *src, const gchar *dst, const gchar *conv, const gchar *write) : m_recursive(false), m_overwrite_target(false), m_create_subdirs(false)
{
