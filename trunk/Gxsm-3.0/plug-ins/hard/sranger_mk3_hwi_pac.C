...
// --- Mux to Output -- N/A w GXSM-PAC
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (pac_bp->button), (s & MD_PLL) ? 1:0);
	g_signal_connect (G_OBJECT (pac_bp->button), "clicked", G_CALLBACK (DSPPACControl::controller_callback), this);

        pac_bp->pop_grid ();
        

        gtk_widget_show_all (pac_bp->grid);
...
//----- Monitoring
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (pac_bp->button), pll.signum_cp_Amp<0. ? 1:0);
	g_signal_connect (G_OBJECT (pac_bp->button), "clicked", G_CALLBACK (DSPPACControl::controller_callback), this);
        pac_bp->new_line ();
...
//	pac_bp->ec->Freeze ();
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (pac_bp->button), pll.signum_cp_Phase<0. ? 1:0);
	g_signal_connect (G_OBJECT (pac_bp->button), "clicked", G_CALLBACK (DSPPACControl::controller_callback), this);
        pac_bp->new_line ();
	...
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (pac_bp->button), pll.signum_ci_Phase<0. ? 1:0);
	g_signal_connect (G_OBJECT (pac_bp->button), "clicked", G_CALLBACK (DSPPACControl::controller_callback), this);
        pac_bp->new_line ();
...
int DSPPACControl::controller_callback (GtkWidget *widget, DSPPACControl *dspc){
	...
	case 0: // Amp Controller SW
		dspc->pll.ctrlmode_Amp = (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) ? 1:0;
	...
	case 1: // Amp Controller CP INV
		dspc->pll.signum_cp_Amp = (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) ? -1.:1.;
		sranger_common_hwi->write_pll( dspc->pll, PLL_CONTROLLER_AMPLITUDE);
		break;
	case 2: // Amp Controller CI INV
		dspc->pll.signum_ci_Amp = (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) ? -1.:1.;
		sranger_common_hwi->write_pll( dspc->pll, PLL_CONTROLLER_AMPLITUDE);
		break;
	case 10: // Phase Controller
		dspc->pll.ctrlmode_Phase = (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) ? 1:0;
	...
	case 11: // Phase Controller CP INV
		dspc->pll.signum_cp_Phase = (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) ? -1.:1.;
		sranger_common_hwi->write_pll( dspc->pll, PLL_CONTROLLER_PHASE);
		break;
	case 12: // Phase Controller CI INV
		dspc->pll.signum_ci_Phase = (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) ? -1.:1.;
		sranger_common_hwi->write_pll( dspc->pll, PLL_CONTROLLER_PHASE);
		break;
	case 100: 
		if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)) && !dspc->refresh_timer_id){
