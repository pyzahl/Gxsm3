...
//	io = 0x0000; WR_GPIO (GPIO_Dir_0, &io, 1);
	...
	for(;;){	/* forever !!! */
		...
		switch (scan.stop){
		case 0: break;
		case AREA_SCAN_STOP:	 scan.stop = 0; scan.pflg = 0; stop_lockin (PROBE_RUN_LOCKIN_SCAN); break; // Stop/Cancel/Abort scan
		case AREA_SCAN_PAUSE:	 scan.stop = 0; scan.pflg &= (AREA_SCAN_START_NORMAL|AREA_SCAN_START_FASTSCAN); break; // Pause Scan
		case AREA_SCAN_RESUME: 
			if (scan.pflg & (AREA_SCAN_START_NORMAL|AREA_SCAN_START_FASTSCAN)){
				scan.stop = 0; scan.pflg |= AREA_SCAN_RUN; 
