...
#define FLAG_AUTO_PLOT   0x02 // auto plot
 ...
 public:
	...
	static int DSP_guru_callback (GtkWidget *widget, DSPControl *dspc);
...
#define MAX_NUM_CHANNELS 26
 ...
 protected:
	void read_dsp_state (); // FB on/off
	void write_dsp_state ();
	
	void conv_dsp_feedback ();
	void read_dsp_feedback (); // cp, cp, setpoint
	void write_dsp_feedback ();
	
	void conv_dsp_analog ();
	void read_dsp_analog (); // bias
	...
	void read_dsp_scan_event_trigger ();
	void write_dsp_scan_event_trigger ();
