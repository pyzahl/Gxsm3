...
// SPM Scan Control Window Title
	...
	PI_DEBUG (DBG_L8, "spm_scancontrol Plugin : building interface -- set_window_geometry" );

        PI_DEBUG (DBG_L2, "spm_scancontrol Plugin : building interface -- start grid..." );

        grid = v_grid;
        
// SPM Control Button Frame
	// --------------------------------------------------
        frame = gtk_frame_new (N_("SPM Scan Control"));
	gtk_grid_attach (GTK_GRID (grid), frame, 1,1, 1,1);

// Start, Movie, (HSC,) Pause, Stop - Action Buttons
        grid_ctrl = gtk_grid_new ();
        x=y=1;
	...
	g_signal_connect ( G_OBJECT (button), "pressed",
	 		     G_CALLBACK (spm_scancontrol_start_callback),
	 		     this);
...
#define ADD_SLS_CONTROL
	...
	GtkWidget *slsbutton = gtk_button_new_with_label("SLS"); 
	gtk_grid_attach (GTK_GRID (grid_ctrl), slsbutton, x++, y, 1,1);
	g_object_set_data (G_OBJECT (slsbutton), "SUBSCAN_MODE", GINT_TO_POINTER (0));
	g_signal_connect (G_OBJECT (slsbutton), "pressed",
	...
	gtk_grid_attach (GTK_GRID (grid_ctrl), button, x++, y, 1,1);
	g_signal_connect ( G_OBJECT (button), "pressed",
			     G_CALLBACK (spm_scancontrol_pause_callback),
			     this);
	...
	gtk_grid_attach (GTK_GRID (grid_ctrl), button, x++, y, 1,1);
	g_signal_connect ( G_OBJECT (button), "pressed",
			     G_CALLBACK (spm_scancontrol_stop_callback),
			     this);
...
// Scan Direction Controls
	...
	radiobutton = gtk_radio_button_new_with_label( NULL, "TopDown");
	gtk_grid_attach (GTK_GRID (grid_ctrl), radiobutton, x, y, 1,1);
	g_object_set_data (G_OBJECT (radiobutton), "SCANDIR", (void*) SCAN_DIR_TOPDOWN);
	g_signal_connect (G_OBJECT (radiobutton), "clicked",
			    G_CALLBACK (cb_setscandir), this);
	
	radiobutton = gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON (radiobutton), "TopDown BotUp");
	gtk_grid_attach (GTK_GRID (grid_ctrl), radiobutton, x+1, y, 2,1);
	g_object_set_data (G_OBJECT (radiobutton), "SCANDIR", (void*) SCAN_DIR_TOPDOWN_BOTUP);
	g_signal_connect (G_OBJECT (radiobutton), "clicked",
			    G_CALLBACK (cb_setscandir), this);
	
	radiobutton = gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON (radiobutton), "BotUp");
	gtk_grid_attach (GTK_GRID (grid_ctrl), radiobutton, x+3, y, 1,1);
	g_object_set_data (G_OBJECT (radiobutton), "SCANDIR", (void*) SCAN_DIR_BOTUP);
	g_signal_connect (G_OBJECT (radiobutton), "clicked",
			    G_CALLBACK (cb_setscandir), this);
	...
	gtk_grid_attach (GTK_GRID (grid_ctrl), checkbutton, x++, y, 1,1);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (checkbutton), MultiVoltMode ());
	g_signal_connect (G_OBJECT (checkbutton), "toggled",
	...
	g_signal_connect (G_OBJECT (slscheckbutton), "clicked",
                            G_CALLBACK (spm_scancontrol_set_subscan_callback),
			    this);
...
//------------ SLS controls
	...
	SPMC_RemoteEntryList = NULL;

#ifdef ADD_SLS_CONTROL
        x=1,++y;
        input = mygtk_grid_add_input (_("SLS Xs"), grid_ctrl, x,y);
        gtk_entry_set_width_chars (GTK_ENTRY (input), 6);
        ec = new Gtk_EntryControl (Unity, _("Out of Range"), &sls_config[0], 0., 99999., ".0f", input, 1., 1.);
        g_object_set_data( G_OBJECT (bs_tmp), "SPMCONTROL_SLS_XS", input);
        SPMC_RemoteEntryList = ec->AddEntry2RemoteList(REMOTE_PREFIX "SLS_Xs", SPMC_RemoteEntryList);
        g_object_set_data (G_OBJECT (slsbutton), "SLSC0", ec);
        g_object_set_data (G_OBJECT (slscheckbutton), "SLSC0", ec);

        input = mygtk_grid_add_input (_("SLS Xn"), grid_ctrl, x,y);
        gtk_entry_set_width_chars (GTK_ENTRY (input), 6);
        ec = new Gtk_EntryControl (Unity, _("Out of Range"), &sls_config[1], 0., 99999., ".0f", input, 1., 1.);
        g_object_set_data( G_OBJECT (bs_tmp), "SPMCONTROL_SLS_XN", input);
        SPMC_RemoteEntryList = ec->AddEntry2RemoteList(REMOTE_PREFIX "SLS_Xn", SPMC_RemoteEntryList);
        g_object_set_data (G_OBJECT (slsbutton), "SLSC1", ec);	
        g_object_set_data (G_OBJECT (slscheckbutton), "SLSC1", ec);

        x=1,++y;
        input = mygtk_grid_add_input (_("SLS Ys"), grid_ctrl, x,y);
        gtk_entry_set_width_chars (GTK_ENTRY (input), 6);
        ec = new Gtk_EntryControl (Unity, _("Out of Range"), &sls_config[2], 0., 99999., ".0f", input, 1., 1.);
        g_object_set_data( G_OBJECT (bs_tmp), "SPMCONTROL_SLS_YS", input);
        SPMC_RemoteEntryList = ec->AddEntry2RemoteList(REMOTE_PREFIX "SLS_Ys", SPMC_RemoteEntryList);
        g_object_set_data (G_OBJECT (slsbutton), "SLSC2", ec);
        g_object_set_data (G_OBJECT (slscheckbutton), "SLSC2", ec);

        input = mygtk_grid_add_input (_("SLS Yn"), grid_ctrl, x,y);
        gtk_entry_set_width_chars (GTK_ENTRY (input), 6);
        ec = new Gtk_EntryControl (Unity, _("Out of Range"), &sls_config[3], 0., 99999., ".0f", input, 1., 1.);
        g_object_set_data( G_OBJECT (bs_tmp), "SPMCONTROL_SLS_YN", input);
        SPMC_RemoteEntryList = ec->AddEntry2RemoteList(REMOTE_PREFIX "SLS_Yn", SPMC_RemoteEntryList);
        g_object_set_data (G_OBJECT (slsbutton), "SLSC3", ec);
...
// Multi-Volt Setup Frame
	...
	// --------------------------------------------------
        ...
        frame = gtk_frame_new (N_("Multi Layer Scan Setup"));
	...
	gtk_container_add (GTK_CONTAINER (frame), scrolled_contents);

        grid_setup = gtk_grid_new ();
	gtk_container_add (GTK_CONTAINER (scrolled_contents), grid_setup);

        x=1, y=-10;

	GtkWidget *lab = gtk_label_new(N_("* Match unit of measure to target"));
	gtk_grid_attach (GTK_GRID (grid_setup), lab, x,y, 4,1);

        x=1;++y;
        input = mygtk_grid_add_spin_input (N_("# Values to use"), grid_setup, x,y, 3);
        ec = new Gtk_EntryControl (Unity, N_("Out of Range"), &multi_volt_number, 1., 999., "3.0f", input, 1., 1.);
        g_object_set_data( G_OBJECT (bs_tmp), "SPMCONTROL_EC_NUM_VALUES", input);

        x=1;++y;
        input = mygtk_grid_add_spin_input (N_("MV Start"), grid_setup, x,y, 3);
        ec = new Gtk_EntryControl (Unity, N_("Out of Range"), &mv_start, -1e3, 1e3, "6.3f", input, 0.1, 0.5);

        x=1;++y;
        input = mygtk_grid_add_spin_input (N_("MV Gap"), grid_setup, x,y, 3);
	ec = new Gtk_EntryControl (Unity, N_("Out of Range"), &mv_gap, 0., 10., "6.3f", input, 0.1, 0.5);

        x=1;++y;
        input = mygtk_grid_add_spin_input (N_("MV End"), grid_setup, x,y, 3);
        ec = new Gtk_EntryControl (Unity, N_("Out of Range"), &mv_end, -1e3, 1e3, "6.3f", input, 0.1, 0.5);

        x=1;++y;
	GtkWidget *gen_button = gtk_button_new_with_label(N_("Generate List"));
	gtk_grid_attach (GTK_GRID (grid_setup), gen_button, x,y, 4,1);
	g_object_set_data( G_OBJECT (gen_button), "GRID_SETUP", grid_setup);
	g_signal_connect (G_OBJECT (gen_button), "clicked",
			  G_CALLBACK (cb_mvolt_generate_list), this);

        x=1;y=0; // !!!
	for (int i=0; i<multi_volt_number; ++i){
		MultiVoltEntry *mve = new MultiVoltEntry (grid_setup, Unity, i);
	...
	gtk_widget_show_all (grid);
        ...
        // save List away...
	g_object_set_data( G_OBJECT (window), "SPMCONTROL_EC_list", EC_list);
...
void SPM_ScanControl::compute_mvolt_list (GtkWidget *grid) {
	...
	for (i=0; i<multi_volt_number; ++i){
		...
		if (i<l){
			...
			MultiVoltEntry *mve = new MultiVoltEntry (grid, Unity, i, v);
			multi_volt_list = g_slist_append (multi_volt_list, mve);
		}
	}
...
//					double bias = ((SPM_ScanControl*)data) -> MultiVoltFromList(i);
				...
				gchar *list[] = { g_strdup ("set"), 
						  g_strdup (gtk_entry_get_text (GTK_ENTRY (((SPM_ScanControl*)data) ->remote_param))), 
...
static void spm_scancontrol_set_subscan_callback (GtkWidget *w, void *data){
        ...
        case 0: ((SPM_ScanControl*)data)->set_sls_mode (TRUE);
                ...
                break;
        case 1:
                ((SPM_ScanControl*)data)->set_sls_mode (s=gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (w)));
                if (s){
