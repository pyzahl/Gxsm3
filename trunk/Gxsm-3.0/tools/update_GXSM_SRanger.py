...
def svn_gxsm_update():
	...
	os.chdir(home_dir)
	if os.path.exists("gxsm_svn"):
	  add_event("Update sourcecode")
	  os.system("svn update svn://svn.code.sf.net/p/gxsm/svn/trunk/Gxsm-2.0 gxsm_svn")
	else:
	  add_event("Checkout sourcecode")
	  os.system("svn checkout svn://svn.code.sf.net/p/gxsm/svn/trunk/Gxsm-2.0 gxsm_svn")
...
def svn_sranger_update():
	...
	add_event("You may have to make inputs there.")
	os.chdir(home_dir)
	if os.path.exists("sranger_svn"):
	  add_event("Update sourcecode")
	  os.system("svn update svn://svn.code.sf.net/p/sranger/svn/trunk/SRanger sranger_svn")
	else:
	  add_event("Checkout sourcecode")
	  os.system("svn checkout svn://svn.code.sf.net/p/sranger/svn/trunk/SRanger sranger_svn")
	os.chdir(home_dir)	
	os.chdir("sranger_svn/SR_STD_SP2/kernel-module")	
	...
	add_event("You may have to make inputs there.")
	os.chdir(home_dir)
	os.chdir("sranger_svn/SR_MK2_MK3Pro/kernel-module")
	add_event("Compiling SRanger kernel modul.")	
	os.system("debuild -i -uc -us -b")
	add_event("Done. Installing via checkinstall.")	
...
def create_main_window():
	...
	event_list = gobject.new(gtk.Label, label="Welcome to the\nGxsm-SRanger-Updater\nVersion " + version + "\n\n")
	...
	button_svn = gtk.Button(label="SVN")
	button_svn_handler = button_svn.connect("clicked", svn_button)
	button_svn.set_flags(gtk.CAN_DEFAULT)
	box2.pack_start(button_svn)
	button_svn.grab_default()
	button_svn.show()
	
	# Button for update via apt-get
	button_apt = gtk.Button(label="APT-GET")
	button_apt_handler = button_apt.connect("clicked", apt_button)
	button_apt.set_flags(gtk.CAN_DEFAULT)
	box2.pack_start(button_apt)
	button_apt.show()

	# Button to cancel
	button_cancel = gtk.Button(label="ABORT")
	button_cancel_handler = button_cancel.connect("clicked", cancel_button)
	button_cancel.set_flags(gtk.CAN_DEFAULT)
	box2.pack_start(button_cancel)
	button_cancel.show()
