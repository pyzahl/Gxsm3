				...
				if (j==0)
					...
					if (!tmp_pc[jj]){
						...
						if(!join_same_x || nas < tmp_pc[jj]->get_scount ()){
							...
							tmp_pc[jj]->scan1d->mem2d->Resize (numx, join_same_x ? nas:1);
							tmp_pc[jj]->AddScan (tmp_pc[jj]->scan1d, 0);
						} else
							tmp_pc[jj]->scan1d->mem2d->Resize (numx, join_same_x ? nas:1);
						
						tmp_pc[jj]->scan1d->data.s.ny = join_same_x ? nas:1;
						tmp_pc[jj]->scan1d->data.s.x0=0.;
						tmp_pc[jj]->scan1d->data.s.y0=0.;
						tmp_pc[jj]->scan1d->data.s.nvalues=1;
						tmp_pc[jj]->scan1d->data.s.ntimes=1;
						tmp_pc[jj]->SetXrange (xmin, xmax);
						
						if (join_same_x && (j-1) >= tmp_pc[jj]->get_scount ()){
							tmp_pc[jj]->AddLine (j-1);
							tmp_pc[jj]->SetTitle (", ", TRUE);
