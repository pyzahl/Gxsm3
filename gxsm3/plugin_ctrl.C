...
static void app_auto_hookup_menu_to_plugin_callback (App *app, const gchar *menusection, const gchar* label, GCallback gc, gpointer user_data=NULL){
        ...
        XSM_DEBUG_GP (DBG_L2, "app_auto_hookup_menu_to_plugin_callback:: Plugin Menu Path requested <%s>  MenuItem= %s ==> label, generated action: [%s] <%s>\n", menusection, label, tmp, tmpaction );

        if (!strcmp (menusection, "windows-section")){
                XSM_DEBUG_GP (DBG_L2, "app_auto_hookup_menu_to_plugin_callback:: *** Skipping Section Windows-Menu. Handled by AppBase automatically.");
        } else {
