...
void gxsm_check_new_user_config()
{
	...
	XsmRescourceManager xrm ("GUI-global", "System");
	if (!xrm.GetStr ("First-Start-Key", NULL)){
		xrm.Put ("First-Start-Key", "key");
		FirstStart = TRUE;
	} else
		FirstStart = xrm.GetBool ("First-Start", TRUE);
	
        // GTK3QQQ
	if(FirstStart || xsmres.force_config ){
		gxsm_search_for_HwI();

		GdkPixbuf *logo      = gdk_pixbuf_new_from_xpm_data (gxsmdruidlogo_xpm);
		GdkPixbuf *watermark = gdk_pixbuf_new_from_xpm_data (gxsmwatermark_xpm);

		gchar *title = 
			g_strdup ( N_("Configure GXSM") );
		gchar *welcome = 
			g_strdup ( N_("Welcome to GXSM!") );
		gchar *intro_text = 
			g_strdup ( N_("You seem to be running GXSM for the first time.\n"
				    "The following steps will set up GXSM by asking you\n"
				    "about all mandatory settings.\n"
				    "\n"
				    "If you are going to setup a SPM, SPA-LEED or\n"
				    "other 2D data acquisition Instrument, you will\n"
				    "need all the calibration parameters, which will\n"
				    "convert the ADC/DAC Voltages to corresponding\n"
				    "physical dimension.\n"
				    "\n"
				    "Note: You can review and change all values later\n"
				    "using the Preferences Dialog.\n"
				    ""
					  ) );
	
		gchar *finish = 
			g_strdup ( N_("Congratulation") );
		gchar *final_text = 
			g_strdup ( N_("GXSM is now ready to go!\n"
				    "\n"
				    "You can review and change all values later\n"
				    "unsig the Preferences Dialog from Settings menu\n"
				    "or starting over GXSM like:\n"
				    "\"gxsm --force-configure\"\n"
				    "This will not replace you previous values with the\n"
				    "defaults! Instead remove \"~/.gnome/gxsm\".\n"
				    "\n"
				    "Have fun,\n       your GXSM developer team.\n"
				    ""
					  ) );

		GnomeResNewUserDruid *my_druid = gnome_res_new_user_druid_new_with_vals (
			xsm_res_def, title, 
			logo, watermark, 
			welcome, intro_text,  
			finish, final_text
		...
		if (gnome_res_new_user_druid_run_druids (my_druid) == GNOME_RES_DRUID_FINISH)
			FirstStart=0;
	
		gnome_res_new_user_druid_destroy (my_druid);
	
//		gobject_... destroy?? (logo);  ??? fixme!
//		gobject_... destroy?? (watermark); ??? fixme!
		g_free (title);
		g_free (welcome);
		g_free (intro_text);
		g_free (finish);
	...
	xrm.PutBool ("First-Start", FirstStart);
#endif
