...
DSPControl::DSPControl () {
	...
	xrm.Get ("mix_gain0", &mix_gain[0], (DSPPACClass) ? "1.0":"0.5");
...
// ==== Folder: Advanced or DSP Expert (advanced Feedback & Scan settings) ========================================
	...
	dsp_bp->grid_add_ec ("FrqRef", Frq, &frq_ref, 10000., 150000., "6g", 0.01, 0.1, "adv-dsp-freq-ref");
        ...
        dsp_bp->grid_add_check_button ("IIR self adaptive on MIX0", "enable self adaptive IIR filtering", 1,
                                       ...
                                       IIR_flag, 1);
        //	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (checkbutton), IIR_flag = IIR_I_crossover > 0 ? 1:0);

	if (!DSPPACClass) { // obsolete on MK3
                dsp_bp->grid_add_check_button ("Random Noise Gen.", "White noise generating add mode");
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (dsp_bp->button), dsp_state_mode & MD_NOISE ? 1:0);
...
// ==== Folder: I-V STS setup ========================================
	...
	gchar *tmptxt = xrm.GetStr ("Probing_PL_remote_set_target", "none");
        ...
        gtk_entry_set_text (GTK_ENTRY (PL_remote_set_target), tmptxt);
...
// ==== Folder: LatMan (lateral manipulation) "GVP" -- now renamed into VP (Vecotr Program) setup ========================================
	...
	// ------------------------------------- VP program table, scrolled
 	GtkWidget *vp = gtk_scrolled_window_new (NULL, NULL);
	...
	gtk_scrolled_window_set_min_content_height  (GTK_SCROLLED_WINDOW (vp), 100);
        ...
        dsp_bp->new_grid ();
	gtk_container_add (GTK_CONTAINER (vp), dsp_bp->grid);
...
// Basic Feedback/Scan Parameter ============================================================
	...
	ncv=sranger_mk2_hwi_ncaddvar (ncf, MK2ID"scan_speed_x",   gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(FastScan_status)) ? "Hz":"A/s", "SRanger: Scan speed X", "Xs Velocity", scan_speed_x);
	ncv->add_att ("label", "Velocity Xs");

	sranger_mk2_hwi_ncaddvar (ncf, MK2ID"fast_scan_flag", "On/Off", "SRanger: Fast-Scan mode (X=sine)", "ldcf", gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(FastScan_status)) ? 1:0);
	...
	sranger_mk2_hwi_ncaddvar (ncf, MK2ID"ldc_flag", "On/Off", "SRanger: Drift-Correct", "ldcf", gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(LDC_status)) ? 1:0);
...
void DSPControl::recalculate_dsp_scan_speed_parameters (gint32 &dsp_scan_dnx, gint32 &dsp_scan_dny, 
	...
	if ( gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(FastScan_status))) 
		dsp_scan_fast_return = -1;
...
int DSPControl::callback_edit_GVP (GtkWidget *widget, DSPControl *dspc){
	...
	if (ki < 0){
		...
		if (dspc->VPprogram[1]){
			for (int j=1; j<10; ++j)
				if (dspc->VPprogram[j]){
					gtk_widget_destroy (dspc->VPprogram[j]);
