...
void sranger_mk2_hwi_spm::ExecCmd(int Cmd){
	...
	case DSP_CMD_APPROCH_MOV_XP: // auto approch "Mover"
	{
		...
		dsp_aap.mover_mode = int_2_sranger_int (AAP_MOVER_XP_AUTO_APP | AAP_MOVER_XP | 
		...
		if (DSPMoverClass->mover_param.MOV_mode & AAP_MOVER_WAVE){
			...
			dsp_aap.piezo_speed = int_2_sranger_int(DSPMoverClass->mover_param.MOV_speed_fac); /* slow down factor */
			if (DSPMoverClass->mover_param.inch_worm_phase > 0.)
				dsp_aap.u_piezo_amp = int_2_sranger_int((int)((double)DSPMoverClass->mover_param.MOV_wave_len * DSPMoverClass->mover_param.inch_worm_phase/360.));
		}else{
	...
	case DSP_CMD_AFM_MOV_YP: // manual move Y+
	{
		...
		dsp_aap.mover_mode = int_2_sranger_int(
			dsp_aap.mover_mode |
			DSPMoverClass->mover_param.MOV_mode | 
			DSPMoverClass->mover_param.MOV_output |
			(DSPMoverClass->mover_param.inch_worm_phase > 0. ? AAP_MOVER_IWMODE : 0)
