...
// enable debug:
#define	DEMO_DEBUG(S) XSM_DEBUG (DBG_L4, S)
#define	DEMO_ERROR(S) XSM_DEBUG_ERROR (DBG_L4, S)
...
/* Construktor for Gxsm demo support base class
 * ==================================================
 * - open device
 * - init things
 * - ...
 */
demo_hwi_dev::demo_hwi_dev(){
	DEMO_DEBUG("open driver");
	DAC_max_points = 1<<15; // DAC X/Y resolution is limiting max scan points...

	dsp = 0;

// Demo Only -- no hardware connected -- dummy code for your convenience only
 ...
 */
demo_hwi_dev::~demo_hwi_dev(){
	DEMO_DEBUG ("closing connection to Demo driver");
	if (dsp)
		close (dsp);
}
...
int demo_hwi_dev::ReadScanData(int y_index, int num_srcs, Mem2d *m[MAX_SRCS_CHANNELS]){
	return 0; // done by thread
}

int demo_hwi_dev::ReadProbeData(int nsrcs, int nprobe, int kx, int ky, Mem2d *m, double scale){
 ...
 */
gchar* demo_hwi_dev::get_info(){
	return g_strdup("*--GXSM Demo HwI class--*\n"
			"no real device connected\n"
			"SPM Simulation Mode");
}

double demo_hwi_dev::GetUserParam (gint n, gchar *id){
	return Demo_SPM_ControlClass->GetUserParam (n, id);
}

gint demo_hwi_dev::SetUserParam (gint n, gchar *id, double value){
	return Demo_SPM_ControlClass->SetUserParam (n, id, value);
}
