...
void SPM_SIM_Control::AppWindowInit(const gchar *title){
        if (title) { // stage 1
                ...
                GtkWidget *mc_popup_menu = gtk_menu_new_from_model (G_MENU_MODEL (gapp->get_hwi_mover_popup_menu ())); // fix me -- reusuing same menu def (from DSP MOVER)
...
void SPM_SIM_Control::create_folder (){
	...
	// save List away...
        ...
        set_window_geometry ("spm-simulator-control"); // must add key to xml file: Gxsm-3.0/gxsm3/org.gnome.gxsm3.window-geometry.gschema.xml
}
