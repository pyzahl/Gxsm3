...
extern kmdsp_hwi_dev *kmdsp_hwi_hardware;
...
#define ADD_EC_WITH_SCALE(LABEL, UNIT, ERROR_TEXT, VAR, LO, HI, FMT, STEP, PAGE, REMOTE_LIST) \
        do{\
		    ...
		    ec->Set_ChangeNoticeFkt(kmdsp_SPM_Control::ChangedNotify, this);\
...
#define ADD_STATUS(LABEL, W, LIST)			\
	do {\
		...
		gtk_entry_set_editable (GTK_EDITABLE (W), FALSE);\
...
#define BUILD_STANDART_PROBE_CONTROLS(HAVE_DUAL, OPTION_FLAGS, OPTION_CB, AUTO_FLAGS, AUTO_CB, EXEC_CB, WRITE_CB, GRAPH_CB, X_CB) \
	do {\
		...
		gtk_box_pack_start (GTK_BOX (hbox_param), button, TRUE, TRUE, 0); \
		gtk_widget_show (button); \
		g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (kmdsp_SPM_Control::Probing_save_callback), this); \
									\
		if (0){ \
...
kmdsp_SPM_Control::kmdsp_SPM_Control ()
{
	int i,j;
	AmpIndex  AmpI;
	GSList *EC_list=NULL;
...
//	read_dsp_scan_event_trigger ();
	...
	XsmRescourceManager xrm("kmdsp_hwi_control");
	...
	SetPtUnit = kmdsp_hwi_pi.app->xsm->MakeUnit (xsmres.daqZunit[0], xsmres.daqZlabel[0]);

	probe_fname = g_strdup ("probe_test.data");
	probe_findex = 0;

	gchar *tmp = g_strdup_printf ("%s [%s]", N_("kmdsp SPM Control"), xsmres.DSPDev);
	AppWindowInit(tmp);
...
//	gtk_box_pack_start (GTK_BOX (vbox), box, TRUE, TRUE, 0);
	...
	g_signal_connect (G_OBJECT (checkbutton), "clicked",
			    G_CALLBACK (kmdsp_SPM_Control::feedback_callback), this);
	...
	g_signal_connect (G_OBJECT (menuitem), "activate",
			    G_CALLBACK (kmdsp_SPM_Control::auto_probe_callback),
			    this);
#endif
...
//	ADD_EC_SPIN_XTERN ("ScanSpdReal", Speed, OUT_OF_RANGE, &scan_speed_x, 0., 1e9, "5g", 1., 10.,  RemoteEntryList, ec->Freeze ());
	...
	for(j=0; j<6; j++){
		...
		for(i=0; i<GAIN_POSITIONS; i++){
			...
			g_signal_connect (G_OBJECT (menuitem), "activate",
					    G_CALLBACK (kmdsp_SPM_Control::choice_Ampl_callback),
					    this);
		}
...
// ==== Folder: Scan Events Trigger ========================================
	...
	gtk_widget_show (checkbutton);
	g_signal_connect (G_OBJECT (checkbutton), "clicked",
			    G_CALLBACK (kmdsp_SPM_Control::se_auto_trigger_callback), this);
...
//	read_dsp_state ();
	...
	g_signal_connect(expert_checkbutton, "toggled",G_CALLBACK(kmdsp_SPM_Control::DSP_expert_callback), this);
	...
	g_signal_connect(guru_checkbutton, "toggled", G_CALLBACK(kmdsp_SPM_Control::DSP_guru_callback), this);
	...
	g_signal_connect(slope_checkbutton, "toggled",G_CALLBACK(kmdsp_SPM_Control::DSP_slope_callback), this);
	...
	g_signal_connect(cret_checkbutton, "toggled",G_CALLBACK(kmdsp_SPM_Control::DSP_cret_callback), this);
...
// ==== Folder: I-V STS setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (TRUE, IV_option_flags, callback_change_IV_option_flags, IV_auto_flags,  callback_change_IV_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_IV_callback, kmdsp_SPM_Control::Probing_write_IV_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: FZ (Force-Distance/AFM) setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (TRUE, FZ_option_flags, callback_change_FZ_option_flags, FZ_auto_flags, callback_change_FZ_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_FZ_callback, kmdsp_SPM_Control::Probing_write_FZ_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: PL (Puls) setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (FALSE, PL_option_flags, callback_change_PL_option_flags, PL_auto_flags,  callback_change_PL_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_PL_callback, kmdsp_SPM_Control::Probing_write_PL_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: LP (Laserpuls) setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (FALSE, LP_option_flags, callback_change_LP_option_flags, LP_auto_flags,  callback_change_LP_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_LP_callback, kmdsp_SPM_Control::Probing_write_LP_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: SP (Special/Slow Puls) setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (FALSE, SP_option_flags, callback_change_SP_option_flags, SP_auto_flags,  callback_change_SP_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_SP_callback, kmdsp_SPM_Control::Probing_write_SP_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: TS (Time Spectroscopy) setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (FALSE, TS_option_flags, callback_change_TS_option_flags, TS_auto_flags,  callback_change_TS_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_TS_callback, kmdsp_SPM_Control::Probing_write_TS_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: LatMan (lateral manipulation) setup ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (TRUE, LM_option_flags, callback_change_LM_option_flags, LM_auto_flags, callback_change_LM_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_LM_callback, kmdsp_SPM_Control::Probing_write_LM_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: LockIn  ========================================
	...
	BUILD_STANDART_PROBE_CONTROLS (TRUE, AC_option_flags, callback_change_AC_option_flags, AC_auto_flags, callback_change_AC_auto_flags,
				       kmdsp_SPM_Control::LockIn_exec_callback, kmdsp_SPM_Control::LockIn_write_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// 			    G_CALLBACK (kmdsp_SPM_Control::LockIn_read_callback), this);
...
// 	gtk_box_pack_start (GTK_BOX (hbox_param), button, TRUE, TRUE, 0);
// 	g_signal_connect (G_OBJECT (button), "clicked",
// 			    G_CALLBACK (kmdsp_SPM_Control::LockIn_write_callback), this);
// 	expert_list = g_slist_prepend (expert_list, button);
...
//	ADD_EC_SPIN ("Resolution", Time, OUT_OF_RANGE, &AX_resolution, 0., 10., "5.3g", 0.001, 0.01,  RemoteEntryList);
	...
	BUILD_STANDART_PROBE_CONTROLS (TRUE, AX_option_flags, callback_change_AX_option_flags, AX_auto_flags,  callback_change_AX_auto_flags,
				       kmdsp_SPM_Control::Probing_exec_AX_callback, kmdsp_SPM_Control::Probing_write_AX_callback, kmdsp_SPM_Control::Probing_graph_callback,
				       kmdsp_SPM_Control::Probing_abort_callback);
...
// ==== Folder: Graphs -- Vector Probe Data Visualisation setup ========================================
	...
	gtk_box_pack_start (GTK_BOX (hbox_param), button, TRUE, TRUE, 0);
	gtk_widget_show (button);
	g_signal_connect (G_OBJECT (button), "clicked", G_CALLBACK (kmdsp_SPM_Control::Probing_graph_callback), this);
	...
	gtk_widget_show (button);
	g_signal_connect (G_OBJECT (button), "clicked",
			    G_CALLBACK (kmdsp_SPM_Control::Probing_save_callback), this);
...
// ------------------------------------------------------------
	kmdsp_hwi_pi.app->RemoteEntryList = g_slist_concat (kmdsp_hwi_pi.app->RemoteEntryList, RemoteEntryList);
...
kmdsp_SPM_Control::~kmdsp_SPM_Control (){
	...
	g_slist_foreach(RemoteEntryList, (GFunc) remove, kmdsp_hwi_pi.app->RemoteEntryList);
...
void kmdsp_SPM_Control::store_values (){
	XsmRescourceManager xrm("kmdsp_hwi_control");
...
void kmdsp_SPM_Control::save_values (NcFile *ncf){

	g_print ("kmdsp_SPM_Control::save_values\n");
	gchar *i=NULL;
	gchar *hwii=NULL;
	if (kmdsp_hwi_hardware)
		hwii = kmdsp_hwi_hardware->get_info ();

	if (IS_AFM_CTRL) // AFM (linear)
		i = g_strconcat ("kmdsp HwI interface: AFM mode selected.\nHardware-Info:\n", hwii, NULL);
	else
		i = g_strconcat ("kmdsp HwI interface: STM mode selected.\nHardware-Info:\n", hwii, NULL);

	NcDim* infod  = ncf->add_dim("kmdsp_info_dim", strlen(i));
	NcVar* info   = ncf->add_var("kmdsp_info", ncChar, infod);
	info->add_att("long_name", "kmdsp HwI plugin information");
	info->put(i, strlen(i));
	g_free (i);
	g_free (hwii);
...
// Basic Feedback/Scan Parameter ============================================================
	...
	NcVar* ncv_bias = ncf->add_var ("kmdsp_hwi_bias", ncDouble);
	ncv_bias->add_att ("long_name", "kmdsp: (Sample or Tip) Bias Voltage");
	...
	if (IS_AFM_CTRL){ // AFM (linear)
		NcVar* ncv_sp = ncf->add_var ("kmdsp_hwi_voltage_set_point", ncDouble);
		ncv_sp->add_att ("long_name", "kmdsp: Feedback Set Point (AFM, linear FB mode)");
		...
		NcVar* ncv_sp = ncf->add_var ("kmdsp_hwi_current_set_point", ncDouble);
		ncv_sp->add_att ("long_name", "kmdsp: Current set point (STM, log FB mode)");
	...
	NcVar* ncv_move_speed = ncf->add_var ("kmdsp_hwi_move_speed_x", ncDouble);
	ncv_move_speed->add_att ("long_name", "kmdsp: Move speed X");
	...
	NcVar* ncv_scan_speed = ncf->add_var ("kmdsp_hwi_scan_speed_x", ncDouble);
	ncv_scan_speed->add_att ("long_name", "kmdsp: Scan speed X");
	...
	NcVar* ncv_usr_cp = ncf->add_var ("kmdsp_hwi_usr_cp", ncDouble);
	ncv_usr_cp->add_att ("long_name", "kmdsp: User CP");
	ncv_usr_cp->add_att ("short_name", "CP");
	ncv_usr_cp->add_att ("var_unit", "1");
	ncv_usr_cp->put (&usr_cp);

	NcVar* ncv_usr_ci = ncf->add_var ("kmdsp_hwi_usr_ci", ncDouble);
	ncv_usr_ci->add_att ("long_name", "kmdsp: User CI");
	ncv_usr_ci->add_att ("short_name", "CI");
	ncv_usr_ci->add_att ("var_unit", "1");
	ncv_usr_ci->put (&usr_ci);

	NcVar* ncv_pre_points = ncf->add_var ("kmdsp_hwi_pre_points", ncDouble);
	ncv_pre_points->add_att ("long_name", "kmdsp: Pre-Scanline points");
	ncv_pre_points->add_att ("short_name", "Pre-S points");
	ncv_pre_points->add_att ("var_unit", "1");
	ncv_pre_points->put (&pre_points);

	NcVar* ncv_Inst_VX = ncf->add_var ("kmdsp_hwi_XSM_Inst_VX", ncDouble);
	ncv_Inst_VX->add_att ("long_name", "FYI only::kmdsp/XSM: Instrument VX (X-gain)");
	ncv_Inst_VX->add_att ("short_name", "VX");
	ncv_Inst_VX->add_att ("var_unit", "1");
	{ double tmp=kmdsp_hwi_pi.app->xsm->Inst->VX (); ncv_Inst_VX->put (&tmp); }

	NcVar* ncv_Inst_VY = ncf->add_var ("kmdsp_hwi_XSM_Inst_VY", ncDouble);
	ncv_Inst_VY->add_att ("long_name", "FYI only::kmdsp/XSM: Instrument VY (Y-gain)");
	ncv_Inst_VY->add_att ("short_name", "VY");
	ncv_Inst_VY->add_att ("var_unit", "1");
	{ double tmp=kmdsp_hwi_pi.app->xsm->Inst->VY (); ncv_Inst_VY->put (&tmp); }

	NcVar* ncv_Inst_VZ = ncf->add_var ("kmdsp_hwi_XSM_Inst_VZ", ncDouble);
	ncv_Inst_VZ->add_att ("long_name", "FYI only::kmdsp/XSM: Instrument VZ (Z-gain)");
	ncv_Inst_VZ->add_att ("short_name", "VZ");
	ncv_Inst_VZ->add_att ("var_unit", "1");
	{ double tmp=kmdsp_hwi_pi.app->xsm->Inst->VZ (); ncv_Inst_VZ->put (&tmp); }

	if (gapp->xsm->Inst->OffsetMode() == OFM_ANALOG_OFFSET_ADDING){
		NcVar* ncv_Inst_VX0 = ncf->add_var ("kmdsp_hwi_XSM_Inst_VX0", ncDouble);
		ncv_Inst_VX0->add_att ("long_name", "FYI only::kmdsp/XSM: Instrument VX0 (X0-gain) used for analog offset adding");
		ncv_Inst_VX0->add_att ("short_name", "VX0");
		ncv_Inst_VX0->add_att ("var_unit", "1");
		{ double tmp=kmdsp_hwi_pi.app->xsm->Inst->VX0 (); ncv_Inst_VX0->put (&tmp); }
		
		NcVar* ncv_Inst_VY0 = ncf->add_var ("kmdsp_hwi_XSM_Inst_VY0", ncDouble);
		ncv_Inst_VY0->add_att ("long_name", "FYI only::kmdsp/XSM: Instrument VY0 (Y0-gain) used for analog offset adding");
		ncv_Inst_VY0->add_att ("short_name", "VY0");
		ncv_Inst_VY0->add_att ("var_unit", "1");
		{ double tmp=kmdsp_hwi_pi.app->xsm->Inst->VY0 (); ncv_Inst_VY0->put (&tmp); }
		
		NcVar* ncv_Inst_VZ0 = ncf->add_var ("kmdsp_hwi_XSM_Inst_VZ0", ncDouble);
		ncv_Inst_VZ0->add_att ("long_name", "FYI only::kmdsp/XSM: Instrument VZ0 (Z0-gain) used for analog offset adding");
		ncv_Inst_VZ0->add_att ("short_name", "VZ0");
		ncv_Inst_VZ0->add_att ("var_unit", "1");
		{ double tmp=kmdsp_hwi_pi.app->xsm->Inst->VZ0 (); ncv_Inst_VZ0->put (&tmp); }
	}

// LockIn ============================================================

	NcVar* ncv_tmp = ncf->add_var ("kmdsp_hwi_AC_amp", ncDouble);
	ncv_tmp->add_att ("long_name", "kmdsp: (Sample) AC Amplitude (LockIn)");
	ncv_tmp->add_att ("var_unit", "Volt");
	ncv_tmp->put (&AC_amp);

	ncv_tmp = ncf->add_var ("kmdsp_hwi_AC_frq", ncDouble);
	ncv_tmp->add_att ("long_name", "kmdsp: (Sample) AC Frequency (LockIn)");
	ncv_tmp->add_att ("var_unit", "Hertz");
	ncv_tmp->put (&AC_frq);

	ncv_tmp = ncf->add_var ("kmdsp_hwi_AC_phaseA", ncDouble);
	ncv_tmp->add_att ("long_name", "kmdsp: (Sample) AC Phase A (LockIn)");
	ncv_tmp->add_att ("var_unit", "deg");
	ncv_tmp->put (&AC_phaseA);

	ncv_tmp = ncf->add_var ("kmdsp_hwi_AC_phaseB", ncDouble);
	ncv_tmp->add_att ("long_name", "kmdsp: (Sample) AC Phase B (LockIn)");
	ncv_tmp->add_att ("var_unit", "deg");
	ncv_tmp->put (&AC_phaseB);

	ncv_tmp = ncf->add_var ("kmdsp_hwi_AC_lockin_avg_cycels", ncInt);
	ncv_tmp->add_att ("long_name", "kmdsp: (Sample) AC LockIn average cycels (LockIn)");
...
// Scan Event Trigger ============================================================
	if (dsp_scan_event_trigger.pflg){
		for (int i=0; i<8; ++i){
			gchar* var = g_strdup_printf ("kmdsp_hwi_Trigger_Xp_at_%d", i);
			NcVar* ncv_trig = ncf->add_var (var, ncShort);
			g_free (var);
			ncv_trig->add_att ("long_name", "kmdsp: Scan Event Xp Trigger at");
			ncv_trig->add_att ("var_unit", "RevIndex");
			ncv_trig->put (&dsp_scan_event_trigger.trig_i_xp[i]);

			if (i<4){
				var = g_strdup_printf ("kmdsp_hwi_Trigger_Xp_Bias_%d", i);
				ncv_trig = ncf->add_var (var, ncDouble);
				g_free (var);
				ncv_trig->add_att ("long_name", "kmdsp: Scan Event Xp Bias");
				ncv_trig->add_att ("var_unit", "Volt");
			} else {
				var = g_strdup_printf ("kmdsp_hwi_Trigger_Xp_Current_%d", i);
				ncv_trig = ncf->add_var (var, ncDouble);
				g_free (var);
				ncv_trig->add_att ("long_name", "kmdsp: Scan Event Xp Current");
				ncv_trig->add_att ("var_unit", "nA");
			}
			ncv_trig->put (&trigger_bias_setpoint_xp[i]);

			var = g_strdup_printf ("kmdsp_hwi_Trigger_Xm_at_%d", i);
			ncv_trig = ncf->add_var (var, ncShort);
			g_free (var);
			ncv_trig->add_att ("long_name", "kmdsp: Scan Event Trigger Xm at");
			ncv_trig->add_att ("var_unit", "RevIndex");
			ncv_trig->put (&dsp_scan_event_trigger.trig_i_xm[i]);

			if (i<4){
				var = g_strdup_printf ("kmdsp_hwi_Trigger_Xm_Bias_%d", i);
				ncv_trig = ncf->add_var (var, ncDouble);
				g_free (var);
				ncv_trig->add_att ("long_name", "kmdsp: Scan Event Xm Bias");
				ncv_trig->add_att ("var_unit", "Volt");
			} else {
				var = g_strdup_printf ("kmdsp_hwi_Trigger_Xm_Current_%d", i);
				ncv_trig = ncf->add_var (var, ncDouble);
				g_free (var);
				ncv_trig->add_att ("long_name", "kmdsp: Scan Event Xm Current");
				ncv_trig->add_att ("var_unit", "nA");
			}
...
void kmdsp_SPM_Control::load_values (NcFile *ncf){
	g_print ("kmdsp_SPM_Control::load_values\n");
	...
	NC_GET_VARIABLE ("kmdsp_hwi_bias", &bias);
	NC_GET_VARIABLE ("kmdsp_hwi_bias", &gapp->xsm->data.s.Bias);
        NC_GET_VARIABLE ("kmdsp_hwi_voltage_set_point", &voltage_set_point);
        NC_GET_VARIABLE ("kmdsp_hwi_current_set_point", &current_set_point);

	update ();
}


#define CONV_16_C(X) kmdsp_hwi_hardware->int_2_kmdsp_int (X)
#define CONV_32_C(X) kmdsp_hwi_hardware->long_2_kmdsp_long (X)

#define CONV_16(X) X = kmdsp_hwi_hardware->int_2_kmdsp_int (X)
#define CONV_32(X) X = kmdsp_hwi_hardware->long_2_kmdsp_long (X)

gint kmdsp_SPM_Control::SetUserParam (gint n, gchar *id, double value){
	return 0;
}

double kmdsp_SPM_Control::GetUserParam (gint n, gchar *id){
	return 0.;
}


void kmdsp_SPM_Control::update(){
...
void kmdsp_SPM_Control::recalculate_dsp_scan_speed_parameters (AREA_SCAN &dsp_scan_par){
	double frac  = (1<<16);
	double fs_dx = frac * kmdsp_hwi_pi.app->xsm->Inst->XA2Dig (scan_speed_x_requested) / frq_ref;
	double fs_dy = frac * kmdsp_hwi_pi.app->xsm->Inst->YA2Dig (scan_speed_x_requested) / frq_ref;
	
 	if ((frac * kmdsp_hwi_hardware->Dx / fs_dx) > (1<<15) || (frac * kmdsp_hwi_hardware->Dy / fs_dx) > (1<<15)){
...
#if 0
	// N: dnx
	dsp_scan_par.dnx = (DSP_INT) ceil (frac * kmdsp_hwi_hardware->Dx * dynamic_zoom / fs_dx);
	dsp_scan_par.dny = (DSP_INT) ceil (frac * kmdsp_hwi_hardware->Dy * dynamic_zoom / fs_dy);
	
	dsp_scan_par.fs_dx = (DSP_LONG) (frac*kmdsp_hwi_hardware->Dx / ceil (frac * kmdsp_hwi_hardware->Dx / fs_dx));
	dsp_scan_par.fs_dy = (DSP_LONG) (frac*kmdsp_hwi_hardware->Dy / ceil (frac * kmdsp_hwi_hardware->Dy / fs_dy));
	
	dsp_scan_par.nx_pre = dsp_scan_par.dnx * pre_points;
	
	dsp_scan_par.fs_dy *= kmdsp_hwi_hardware->scan_direction;


	// re-update to real speed
	scan_speed_x = kmdsp_hwi_pi.app->xsm->Inst->Dig2XA ((DSP_LONG)(dsp_scan_par.fs_dx * frq_ref / frac));
...
void kmdsp_SPM_Control::recalculate_dsp_scan_slope_parameters (AREA_SCAN &dsp_scan_par){
#if 0
	// setup slope compensation parameters 
	if (area_slope_compensation_flag){
		double zx_ratio = kmdsp_hwi_pi.app->xsm->Inst->Dig2XA (1) / kmdsp_hwi_pi.app->xsm->Inst->Dig2ZA (1);
		double zy_ratio = kmdsp_hwi_pi.app->xsm->Inst->Dig2YA (1) / kmdsp_hwi_pi.app->xsm->Inst->Dig2ZA (1);
...
void kmdsp_SPM_Control::updateDSP(int FbFlg){
	if (!kmdsp_hwi_hardware) return; 
...
// mirror basic parameters to GXSM main
	...
	switch(FbFlg){
	case DSP_FB_ON:
		kmdsp_hwi_hardware->ExecCmd(DSP_CMD_START); 
		break;
	case DSP_FB_OFF: 
		kmdsp_hwi_hardware->ExecCmd(DSP_CMD_HALT);
		break;
	}
	...
	if (fabs (ue_bias - bias) > 1e-6){
		kmdsp_hwi_hardware->add_user_event_now (g_strdup_printf (N_("Bias adjust [V]")), ue_bias, bias, addflag);
		ue_bias = bias;
		addflag=TRUE;
	}
	if (fabs (ue_current_set_point - current_set_point) > 1e-6){
		kmdsp_hwi_hardware->add_user_event_now (g_strdup_printf (N_("Current Set Pt. adjust [nA]")), ue_current_set_point, current_set_point, addflag);
		ue_current_set_point = current_set_point;
		addflag=TRUE;
	}
	if (fabs (ue_voltage_set_point - voltage_set_point) > 1e-6){
		kmdsp_hwi_hardware->add_user_event_now (g_strdup_printf (N_("Voltage Set Pt. adjust [V]")), ue_voltage_set_point, voltage_set_point, addflag);
		ue_voltage_set_point = voltage_set_point;
		addflag=TRUE;
	}
	if (fabs (ue_usr_cp - usr_cp) > 1e-6){
		kmdsp_hwi_hardware->add_user_event_now (g_strdup_printf (N_("CP adjust [1]")), ue_usr_cp, usr_cp, addflag);
		ue_usr_cp = usr_cp;
		addflag=TRUE;
	}
	if (fabs (ue_usr_ci - usr_ci) > 1e-6){
		kmdsp_hwi_hardware->add_user_event_now (g_strdup_printf (N_("CI adjust [1]")), ue_usr_ci, usr_ci, addflag);
		ue_usr_ci = usr_ci;
		addflag=TRUE;
	}
...
void kmdsp_SPM_Control::update_trigger(gboolean flg){
	if (flg){
...
int kmdsp_SPM_Control::ChangedAction(GtkWidget *widget, kmdsp_SPM_Control *dspc){
	dspc->updateDSP();
	return 0;
}

void kmdsp_SPM_Control::ChangedNotify(Param_Control* pcs, gpointer dspc){
	//  gchar *us=pcs->Get_UsrString();
	//  PI_DEBUG (DBG_L2, "DSPC: " << us );
	//  g_free(us);
	((kmdsp_SPM_Control*)dspc)->updateDSP();
}

int kmdsp_SPM_Control::feedback_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::se_auto_trigger_callback(GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::LockIn_read_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
//	dspc->read_dsp_probe ();
}

int kmdsp_SPM_Control::LockIn_write_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::LockIn_exec_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
//	kmdsp_hwi_hardware->start_fifo_read (0, 0,0,0,0, NULL,NULL,NULL,NULL);
}

int kmdsp_SPM_Control::Probing_exec_IV_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_IV_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_IV);
}

int kmdsp_SPM_Control::Probing_exec_FZ_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_FZ_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_FZ);
}

// PL

int kmdsp_SPM_Control::Probing_exec_PL_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_PL_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_PL);
}

// LP

int kmdsp_SPM_Control::Probing_exec_LP_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
	dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_LP_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
	dspc->write_dsp_probe (0, PV_MODE_LP);
}

// SP

int kmdsp_SPM_Control::Probing_exec_SP_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_SP_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_SP);
}

// TS

int kmdsp_SPM_Control::Probing_exec_TS_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_TS_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_TS);
}

// LM

int kmdsp_SPM_Control::Probing_exec_LM_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_LM_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::Probing_exec_AX_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
        dspc->write_dsp_probe (0, PV_MODE_NONE);
...
int kmdsp_SPM_Control::Probing_write_AX_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
}


// for TESTING and DEBUGGIG only
int kmdsp_SPM_Control::Probing_exec_RF_callback( GtkWidget *widget, kmdsp_SPM_Control *dspc){
}


int kmdsp_SPM_Control::Probing_graph_callback(GtkWidget *widget, kmdsp_SPM_Control *dspc){
// refresh data view....
	;
}

int kmdsp_SPM_Control::Probing_save_callback(GtkWidget *widget, kmdsp_SPM_Control *dspc){
// save data....
	;
}

void kmdsp_SPM_Control::StartScanPreCheck (){
	dynamic_zoom = 1.;
	update ();
...
int kmdsp_SPM_Control::auto_probe_callback(GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::choice_Ampl_callback (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	AmpIndex i;
	i.l=(DSP_LONG)GPOINTER_TO_INT (g_object_get_data( G_OBJECT (widget), "chindex"));
	switch(i.s.ch){
	case 0: kmdsp_hwi_pi.app->xsm->Inst->VX((int)i.s.x);
		if (gapp->xsm->Inst->OffsetMode() == OFM_DSP_OFFSET_ADDING)
			kmdsp_hwi_pi.app->xsm->Inst->VX0((int)i.s.x);
		break;
	case 1: kmdsp_hwi_pi.app->xsm->Inst->VY((int)i.s.x);
		if (gapp->xsm->Inst->OffsetMode() == OFM_DSP_OFFSET_ADDING)
			kmdsp_hwi_pi.app->xsm->Inst->VY0((int)i.s.x);
		break;
	case 2: kmdsp_hwi_pi.app->xsm->Inst->VZ((int)i.s.x); 
		if (gapp->xsm->Inst->OffsetMode() == OFM_DSP_OFFSET_ADDING)
			kmdsp_hwi_pi.app->xsm->Inst->VZ0((int)i.s.x); 
		break;
	case 3:
		if (gapp->xsm->Inst->OffsetMode() == OFM_ANALOG_OFFSET_ADDING)
			kmdsp_hwi_pi.app->xsm->Inst->VX0((int)i.s.x);
	...
	kmdsp_hwi_pi.app->spm_range_check(NULL, kmdsp_hwi_pi.app);
	dspc->updateDSP();
	return 0;
}

int kmdsp_SPM_Control::callback_change_AC_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::callback_change_AC_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_IV_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_IV_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_FZ_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_FZ_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_PL_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_PL_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
...
int kmdsp_SPM_Control::callback_change_LP_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_LP_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_SP_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_SP_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_TS_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = (DSP_LONG) GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_TS_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_LM_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_LM_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_AX_option_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::callback_change_AX_auto_flags (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long msk = GPOINTER_TO_INT (g_object_get_data(G_OBJECT(widget), "Bit_Mask"));
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::DSP_expert_callback (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::DSP_guru_callback (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget)))
...
int kmdsp_SPM_Control::DSP_slope_callback (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	dspc->area_slope_compensation_flag = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget));	
	dspc->updateDSP();
}

int kmdsp_SPM_Control::DSP_cret_callback (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	dspc->center_return_flag = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget));	
}

int kmdsp_SPM_Control::change_source_callback (GtkWidget *widget, kmdsp_SPM_Control *dspc){
	long channel;
