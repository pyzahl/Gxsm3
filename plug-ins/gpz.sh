...
echo Possible Values: Convert, Arithmetic, Background, 
echo                  Filter 1D, Filter 2D, Statistics, Transformations, Misc
DUMMY=""; read DUMMY
if test -n "$DUMMY" ; then 
  ENTRYPLACE=$DUMMY 
fi
...
#	calculate MENUPATH
MENUPATH="Math/$ENTRYPLACE/"

#	calculate pluginname 
if [ "$ENTRYPLACE" = "Background" ]; then
  SHORTENTRYPLACE="BG"
  TARGETDIRECTORY="math/background"  
elif [ "$ENTRYPLACE" = "Convert" ]; then
  SHORTENTRYPLACE="CV"
  TARGETDIRECTORY="math/convert"  
elif [ "$ENTRYPLACE" = "Arithmetic" ]; then
  SHORTENTRYPLACE="AR"
  TARGETDIRECTORY="math/arithmetic"  
elif [ "$ENTRYPLACE" = "Filter 1D" ]; then
  SHORTENTRYPLACE="F1D"
  TARGETDIRECTORY="math/filter1d"  
elif [ "$ENTRYPLACE" = "Filter 2D" ]; then
  SHORTENTRYPLACE="F2D"
  TARGETDIRECTORY="math/filter2d"  
elif [ "$ENTRYPLACE" = "Statistics" ]; then
  SHORTENTRYPLACE="ST"
  TARGETDIRECTORY="math/statistik"  
elif [ "$ENTRYPLACE" = "Transformations" ]; then
  SHORTENTRYPLACE="TR"
  TARGETDIRECTORY="math/transform"  
elif [ "$ENTRYPLACE" = "Misc" ]; then
...
echo lib"$PLUGINNAME"_la_LDFLAGS = -export-dynamic -avoid-version >> $TARGETDIRECTORY/Makefile.am

#
#	create source-file, based on extraction from this file
#
tail -n 283 $0 > $TARGETDIRECTORY/"$PLUGINNAME".TEMP1
...
#%
#%% PlugInUsage
#%Call \GxsmMenu{___MENUPATH___PLUGINMENUENTRY}.
#%
